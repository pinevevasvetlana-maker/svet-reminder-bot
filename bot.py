...
def parse_datetime(date_part: str, time_part: str) -> Optional[datetime]:
    try:
        return datetime.strptime(f"{date_part} {time_part}", DATETIME_FORMAT).replace(
            tzinfo=timezone.utc
        )
    except ValueError:
        return None
...
def build_reminder_line(reminder: Reminder) -> str:
    ...
    return (
        f"#{reminder.reminder_id} на {reminder.remind_at.strftime(DATETIME_FORMAT)} UTC"
...
async def remindme(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        f"Напоминание запланировано на {remind_at.strftime(DATETIME_FORMAT)} UTC."
    )
...
async def repeatme(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        f"Старт: {remind_at.strftime(DATETIME_FORMAT)} UTC, интервал: {interval_minutes} мин."
    )
